...
class AccountExpenseHistory:
    ...
    def check_exclusion(self, *, date, expense, location):
        """
        Checks exclusions rules, returns True if the expense should be ignored, otherwise False
        ...
        """ 
        # Ignore payments/deposits to account 
        if (expense < 0):
            return True
        # Ignore balance transfers
        elif (location.startswith("BALANCE TRANSFER")):
            return True
        else:
            return False
...
class BudgetAnalyzerApp(App):
    ...
    def build(self):
        # Data paths 
        data_dir = PROJ_DIR + os.path.sep + "Data" + os.path.sep + "Demo Data"

        eric_data    = data_dir + os.path.sep + "Eric"
        michael_data = data_dir + os.path.sep + "Michael"
        graham_data  = data_dir + os.path.sep + "Graham"
        john_data    = data_dir + os.path.sep + "John"
        terry_data   = data_dir + os.path.sep + "Terry"

        # Configuration paths
        config_dir = PROJ_DIR + os.path.sep + "Config" + os.path.sep + "Demo Config"

        eric_config    = config_dir + os.path.sep + "Eric"
        michael_config = config_dir + os.path.sep + "Michael"
        graham_config  = config_dir + os.path.sep + "Graham"
        john_config    = config_dir + os.path.sep + "John"
        terry_config   = config_dir + os.path.sep + "Terry"

        
        # Expense histories

        graham_expenses = AccountExpenseHistory(data_path = graham_data, label = "Graham")
        
        eric_expenses = AccountExpenseHistory(data_path = eric_data, label = "Eric")
        
        john_expenses = AccountExpenseHistory(data_path = john_data, label = "John")

        michael_expenses = AccountExpenseHistory(data_path = michael_data, label = "Michael")

        terry_expenses = AccountExpenseHistory(data_path = terry_data, label = "Terry")

        account_tuple = (graham_expenses,
                         eric_expenses,
        		 john_expenses,
                         michael_expenses,
                         terry_expenses )
        ...
        # Analyzers
        graham = ExpenseHistoryAnalyzer (
                          config_path = graham_config,
                   target_total_limit = 800,
                                label = "Graham",
                               graham = graham_expenses)

        eric = ExpenseHistoryAnalyzer (
                          config_path = eric_config,
                   target_total_limit = 1000,
                                label = "Eric",
                                 eric = eric_expenses)

        michael = ExpenseHistoryAnalyzer (
                          config_path = michael_config,
                   target_total_limit = 800,
                                label = "Michael",
                              michael = michael_expenses)

        john = ExpenseHistoryAnalyzer (
                          config_path = john_config,
                   target_total_limit = 900,
                                label = "John",
                               john = john_expenses)

        pythons = ExpenseHistoryAnalyzer (
                          config_path = graham_config,
                   target_total_limit = 800,
                                label = "pythons",
              intersect_account_dates = False,
                               graham = graham_expenses,
                                 john = john_expenses,
                              michael = michael_expenses,
                                 eric = eric_expenses,
                                terry = terry_expenses)

        analyzer_tuple = (graham, 
                            eric, 
                            john, 
                         michael, 
                         pythons )
